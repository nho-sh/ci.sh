...

{
    ...
    if [ "$SETUP_EXIT_CODE" -eq 0 ]; then
        ...
        if [ "$_CISH_CI_SUITE" = 'teamcity' ]; then
            ...
            echo "##teamcity[message text='Setup failed in $(_cishGetLastSection)' errorDetails='See lines above' status='ERROR']"

            # TODO: add gitlab/github

        else
            cishLog "$_CISH_EMOJI_ERROR Setup failed in $(_cishGetLastSection)"
    ...
    if [ "$SETUP_EXIT_CODE" = "0" ]; then
        ...
        if [ "$RUN_EXIT_CODE" = "0" ]; then
            ...
            if [ "$_CISH_CI_SUITE" = 'teamcity' ]; then
                # Teamcity compatible message
                # https://www.jetbrains.com/help/teamcity/service-messages.html#Reporting+Build+Status
                echo "##teamcity[message text='Run failed in $(_cishGetLastSection)' errorDetails='See lines above' status='ERROR']"

                # TODO: add gitlab/github

            else
                cishLog "$_CISH_EMOJI_ERROR Run failed in $(_cishGetLastSection)"
...

{
    ...
    if [ "$TEARDOWN_EXIT_CODE" = "0" ]; then
        ...
        if [ "$_CISH_CI_SUITE" = 'teamcity' ]; then
            # Teamcity compatible message
            # https://www.jetbrains.com/help/teamcity/service-messages.html#Reporting+Build+Status
            echo "##teamcity[message text='Teardown failed in $(_cishGetLastSection)' errorDetails='See lines above' status='ERROR']"

            # TODO: add gitlab/github

        else
            cishLog "$_CISH_EMOJI_ERROR Teardown failed in $(_cishGetLastSection)"
